...
def get_ping_loss(os_name, ping_output):
...
def get_ping_rtt(os_name, ping_output):
    """
    ...
    if os_name == 'windows':
        ...
        pattern = r"""
        (?:min/avg/max/(?:mdev|stddev))  # Palabras clave
        \s*                              # Espacios  
        =                                # Símbolo igual
        \s*                              # Espacios  
        [.\d]+                           # Valor min
...
def get_traceroute_hops(traceroute_output, probes_per_hop=3):
    """
...
def execute_traceroute(host, max_hops=30,
    ...
    if not errors:
        if not output:
            errors = 'El comando no devolvió resultados'
        elif not hops:
